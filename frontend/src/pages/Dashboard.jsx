...
export default function Dashboard({ t, isRtl, currentPage, selectedCourseId, setSelectedCourseId, setCurrentPage, selectedComponentsForQuiz, setSelectedComponentsForQuiz, onRefreshReady }) {
  ...
  const handleSaveComponent = async (courseId, topicName = null) => {
    ...
    if (topicToSave) {
      ...
      try {
        ...
        setComponentError(isRtl ? 'حدث خطأ أثناء الحفظ. يرجى المحاولة مرة أخرى.' : 'An error occurred while saving. Please try again.');
      } finally {
            ...
            {isAddingComponent ? (
              <div style={{ marginTop: '10px', display: 'flex', flexDirection: 'column', gap: '12px', background: 'rgba(0,0,0,0.02)', padding: '15px', borderRadius: '12px', border: '1px solid rgba(0,0,0,0.05)' }}>
                {(isSuggestingComponents || suggestedComponents.length > 0) && (
                  <div style={{ padding: '12px', background: 'rgba(59, 130, 246, 0.05)', borderRadius: '10px', border: '1px solid rgba(59, 130, 246, 0.15)' }}>
                    <h4 style={{ margin: '0 0 8px', fontSize: '13px', color: '#1e3a8a', display: 'flex', alignItems: 'center', gap: '6px' }}>
                      ...
                      {isSuggestingComponents ? (isRtl ? 'جاري اقتراح مواضيع...' : 'AI suggestions loading...') : (isRtl ? 'مقترحات الذكاء الاصطناعي — انقر للإضافة' : 'AI Suggestions — click to add')}
                    </h4>
                    {!isSuggestingComponents && suggestedComponents.length > 0 && (
                ...
                <input
                  ...
                  placeholder={isRtl ? 'أو أدخل موضوعاً يدوياً...' : 'Or type a topic manually...'}
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button className="btn-luxe primary hover-lift" disabled={isValidatingComponent || !newComponentName.trim()} onClick={() => handleSaveComponent(selectedCourse.id)} style={{ padding: '8px', flex: 1, justifyContent: 'center' }}>
                    {isValidatingComponent ? <Loader2 size={16} className="spin-icon" /> : (t.save || 'Save')}
                  </button>
                  <button className="btn-luxe hover-lift" disabled={isValidatingComponent} onClick={() => { setIsAddingComponent(false); setNewComponentName(''); setSuggestedComponents([]); setComponentError(''); }} style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', color: '#475569', padding: '8px', flex: 1, justifyContent: 'center' }}>{t.cancel || 'Cancel'}</button>
                </div>
              </div>
            ) : (
            ...
            <div style={{ marginTop: '12px', position: 'relative' }}>
              ...
              >
                <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  {isRtl ? 'إنشاء مادة دراسية' : 'Generate Study Aid'}
                </span>
                <span style={{ fontSize: '12px', opacity: 0.6, transform: showStudyAidMenu ? 'rotate(180deg)' : 'none', display: 'inline-block', transition: 'transform 0.2s' }}>▼</span>
              </button>
                ...
                <div style={{ position: 'absolute', top: 'calc(100% + 6px)', left: 0, right: 0, background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '12px', boxShadow: '0 10px 30px rgba(0,0,0,0.1)', zIndex: 50, overflow: 'hidden' }}>
                  ...
                  >
                    ...
                    <div style={{ textAlign: isRtl ? 'right' : 'left' }}>
                      ...
                      <div style={{ fontSize: '12px', color: '#94a3b8', fontWeight: '400' }}>{isRtl ? 'ملخص نصي للمواضيع المختارة' : 'Text summary of selected topics'}</div>
                    </div>
                  </button>
                  <button
                  ...
                  >
                    ...
                    <div style={{ textAlign: isRtl ? 'right' : 'left' }}>
                      ...
                      <div style={{ fontSize: '12px', color: '#94a3b8', fontWeight: '400' }}>{isRtl ? 'خريطة ذهنية تفاعلية' : 'Visual interactive mind map'}</div>
                    </div>
                  </button>
                </div>
              )}
