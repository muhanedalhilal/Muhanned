  ...
  const renderCourseCard = (course) => {
    ...
    return (
      <div key={course.id} className="luxe-card clickable hover-lift animate-slide-up glass-glow" onClick={() => setSelectedCourseId(course.id)} style={{ overflow: 'hidden' }}>
        {course.image_url ? (
          <div style={{ width: '100%', height: '100px', marginBottom: '12px', borderRadius: '12px', overflow: 'hidden', background: `linear-gradient(135deg, ${course.color}20, ${course.color}40)` }}>
          ...
          <div className="form-body" style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <div style={{ position: 'relative' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '14px 16px', borderRadius: '12px', border: newCourseFile ? '2px solid #3b82f6' : '1px dashed #cbd5e1', background: newCourseFile ? 'rgba(59, 130, 246, 0.04)' : '#fff', cursor: 'pointer', transition: 'all 0.2s' }}>
                ...
                <span style={{ fontSize: '14px', color: newCourseFile ? '#1e293b' : '#64748b', fontWeight: '500' }}>{newCourseFile ? newCourseFile.name : (t.addResourcesOptional || 'Add Resources (PDF/PPTX) — Optional')}</span>
                <input type="file" accept=".pdf,.pptx" style={{ display: 'none' }} onChange={(e) => setNewCourseFile(e.target.files?.[0] || null)} />
                {newCourseFile && (<button type="button" onClick={(ev) => { ev.preventDefault(); ev.stopPropagation(); setNewCourseFile(null); }} style={{ marginLeft: 'auto', background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', padding: '2px' }}><Trash2 size={16} /></button>)}
              </label>
              <span style={{ position: 'absolute', top: '-8px', right: '12px', background: '#f8fafc', padding: '0 6px', fontSize: '11px', color: '#94a3b8', fontWeight: '500' }}>{t.optionalField || 'Optional'}</span>
            </div>
